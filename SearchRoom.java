...
public class SearchRoom extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (roomtype.equals("Room Type")|| roomposition.equals("Room Position")) {
                out.println("Field cannot be Empty");
            } else {
                HotelBean bean = new HotelBean();
                bean.setRoomtype(roomtype);
                bean.setRoomposition(roomposition);
                HotelHelper helper = new HotelHelper();
                ResultSet rst = helper.checkRoom(bean);

                try {
                    if (rst.next()) {
                        out.println("Room Available");
                    } else {
